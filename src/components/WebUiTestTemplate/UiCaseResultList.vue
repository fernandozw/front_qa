<template>
  <div>
    <el-form :inline="true" :model="searchForm" size="mini">
      ...
      <el-form-item label="用例id">
        <el-input
          v-model="searchForm.caseIds"
          placeholder="输入用例id,多个id以','分割"
          clearable
        ></el-input>
      </el-form-item>
    ...
    >
      ...
      <el-table-column label="失败元素" width="250" show-overflow-tooltip>
        <template slot-scope="scope">
          <el-button
            v-if="
              typeof tableData[scope.$index].elementId === 'number' &&
                !isNaN(tableData[scope.$index].elementId)
            "
            type="text"
            size="mini"
      ...
      <el-table-column label="失败步骤" width="250" show-overflow-tooltip>
        <template slot-scope="scope">
          <el-button
            v-if="
              typeof tableData[scope.$index].failStepId === 'number' &&
                !isNaN(tableData[scope.$index].failStepId)
            "
            type="text"
            size="mini"
