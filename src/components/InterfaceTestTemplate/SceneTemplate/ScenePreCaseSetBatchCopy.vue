      ...
      <el-card class="box-card">
        <el-form ref="copyForm" :model="copyForm" :rules="rules" :inline="true">
          <el-row :gutter="40">
            ...
            <el-col :span="12">
              <el-form-item label="源版本" prop="source_version_id">
                <el-select
                  v-model="copyForm.source_version_id"
                  clearable
                  placeholder="请选择"
                  filterable
                  size="mini"
          ...
          <el-row :gutter="40">
            <el-col :span="8">
              <el-form-item label="目标版本" prop="version_id">
                <el-select
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        pre_fix: [{ required: true, message: '名称前缀不能为空' }],
        source_version_id: [{ required: true, message: '源版本不能为空' }],
        project_id: [{ required: true, message: '所属项目不能为空' }],
        create_person: [{ required: true, message: '创建人不能为空' }],
        version_id: [{ required: true, message: '目标版本不能为空' }]
      },
      copyVisible: false
    };
  },
  methods: {
    ...
    getConditionsForCopy() {
      this.copyForm.source_version_id = '';
      this.copyForm.version_id = '';
      let url = String.format(
        '{0}/sceneInterface/sceneConditions?project_id={1}',
        this.GLOBAL.MONITOR_URL,
        this.copyForm.project_id
        ...
        if (valid) {
          if (
            ...
            if (
              ...
              let url = String.format(
                '{0}/sceneInterface/batchCopyScene',
                this.GLOBAL.MONITOR_URL
              );
              // 指定参数格式
              let config = {
