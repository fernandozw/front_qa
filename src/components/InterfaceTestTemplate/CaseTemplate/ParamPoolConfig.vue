    ...
    <el-form :model="conditionDict" :inline="true" class="demo-form-inline">
      <el-row>
        ...
        <el-form-item label="项目版本:">
          <el-select
            v-model="conditionDict.version_id"
            clearable
            placeholder="请选择"
            filterable
            size="mini"
        ...
        <el-form ref="addForm" :model="addForm" :rules="rules" :inline="true">
          <el-row :gutter="10">
            ...
            <el-col :span="6" :pull="1">
              <el-form-item label="项目版本:" prop="version_id">
                <el-select
                  v-model="addForm.version_id"
                  clearable
                  placeholder="请选择"
                  filterable
                  size="mini"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        version_id: [{ required: true, message: '项目版本不能为空' }]
      },
      editRules: {
        pool_name: [{ required: true, message: '参数池名称不能为空' }],
        project_id: [{ required: true, message: '所属项目不能为空' }],
  ...
  methods: {
    ...
    changeProject(projectId, type) {
      if (type === 'add') {
        this.addForm.version_id = '';
      } else if (type === 'edit') {
        this.editForm.version_id = '';
      } else if (type === 'list') {
        this.conditionDict.version_id = '';
      }
    ...
    clickInit(type) {
      ...
      let version = '';
      if (type === 'add') {
        version = this.addForm.version_id;
      } else if (type === 'edit') {
        version = this.editForm.version_id;
      }
      if (version === '') {
